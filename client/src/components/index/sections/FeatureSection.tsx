  ...
  return (
    <Card className="group transition transition-shadow-200 hover:shadow-xl bg-white">
      <CardContent className="p-5 flex gap-4">
        <div className="shrink-0 w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center text-green-700">{icon}</div>

        <div className="space-y-1">
          <h4 className="text-sm font-semibold text-h3">{title}</h4>
...
function StepCard({ n, title, desc }: { n: string; title: string; desc: string }) {
  return (
    <Card className="relative overflow-hidden hover:shadow-xl bg-white">
    ...
    <section id={id} className={`px-6 py-20 border-t border-border ${variant === "steps" ? "bg-surface-muted" : ""}`}>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-12 space-y-2">
          <span className="text-overline text-h1">{overline}</span>
